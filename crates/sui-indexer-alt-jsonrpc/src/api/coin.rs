...
use sui_indexer_alt_schema::objects::{StoredCoinBalanceBucket, StoredObject};
...
impl CoinServer {
    async fn get_coins_impl(
        ...
        let mut query = query!("
            SELECT * FROM (
            SELECT DISTINCT ON (object_id) * FROM coin_balance_buckets ORDER BY object_id, cp_sequence_number DESC
        ) AS latest
        ");
        ...
        if let Some(coin_type_tag) = coin_type_tag {
            ...
            // query = query.filter(cb::coin_type.eq(serialized_coin_type));
        }

        if let Some(cursor) = cursor {
            let cursor_coin_query = query!(format!(
                "SELECT * FROM coin_balance_buckets WHERE object_id = '\\x{}'::bytea",
                hex::encode(cursor)
            ));
            ...
            let cursor_balance_bucket = cursor_coin.coin_balance_bucket.unwrap();
        ...
        query = query
            .order_by("coin_balance_bucket DESC")
            .order_by("object_id ASC")
            // Query one more result to determine if there is a next page.
            .limit((limit + 1) as i64);

        // query = query.order_by(cb::coin_balance_bucket.desc()).order_by(cb::object_id.asc()).limit((limit + 1) as i64);

        let buckets: Vec<StoredCoinBalanceBucket> = conn.results_with_raw_query(query).await?;
